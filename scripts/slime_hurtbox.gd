...
@onready var animated_sprite: AnimatedSprite2D = slime.get_node("AnimatedSprite2D")

# Attack frames where the hurtbox should be active (adjust as needed)
# Slime attack animation frames - typically middle frames when it lunges
...
func _check_overlapping_areas() -> void:
	...
	if overlapping_areas.size() == 0 and slime.player:
		var player_hitbox = slime.player.get_node_or_null("player_hitbox/hitbox")
		if player_hitbox:
...
func _on_area_entered(area: Area2D) -> void:
	...
	if player and player.has_method("take_knockback"):
		...
		var knockback_direction = (player.global_position - slime.global_position).normalized()
