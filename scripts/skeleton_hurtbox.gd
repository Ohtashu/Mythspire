...
# skeleton_attack has 6 frames (0-5), frame 2 is where damage happens
const ATTACK_ACTIVE_FRAMES = [2]
...
func _process(_delta: float) -> void:
	# Safety check: Disable monitoring if skeleton is defending or not in attack state
	if skeleton.is_defending or skeleton.current_state != skeleton.State.ATTACK:
		hurtbox.monitoring = false
...
func _on_frame_changed() -> void:
	...
	# Also check that skeleton is not defending and is in ATTACK state
	if animated_sprite.animation == "skeleton_attack" and not skeleton.is_defending and skeleton.current_state == skeleton.State.ATTACK:
		# Enable monitoring only on the active swing frame (frame 2)
		...
		# Ensure monitoring is OFF for all other animations (idle, walk, chase, defend, etc.)
		hurtbox.monitoring = false
		check_overlaps_next_frame = false
...
func _on_area_entered(area: Area2D) -> void:
	...
	if animated_sprite.frame not in ATTACK_ACTIVE_FRAMES:
		return  # Only deal damage on frame 2
	...
	if player and player.has_method("take_knockback"):
		...
		# Deal damage to player (reduced if defending)
