...
@onready var hurtbox: Area2D = $hurtbox2
@onready var evil_sword = get_parent()
@onready var animated_sprite: AnimatedSprite2D = evil_sword.get_node("AnimatedSprite2D")

# Attack frames where the hurtbox should be active (adjust as needed)
# Evil sword attack animation frames - typically middle frames when it swings
const ATTACK_ACTIVE_FRAMES = [2, 3, 4]
...
func _on_frame_changed() -> void:
	...
	if animated_sprite.animation == "sword_attack":
		# Enable monitoring only on the active attack frames
...
func _check_overlapping_areas() -> void:
	...
	# Get evil sword hurtbox shape info
	...
	if hurtbox_shape and hurtbox_shape.shape is RectangleShape2D:
		var size = hurtbox_shape.shape.size
		hurtbox_radius = max(size.x, size.y) / 2.0
	
	# If no overlaps found but player is close, manually check distance and apply knockback
	if overlapping_areas.size() == 0 and evil_sword.player:
		var player_hitbox = evil_sword.player.get_node_or_null("player_hitbox/hitbox")
		if player_hitbox:
...
func _on_area_entered(area: Area2D) -> void:
	...
	if evil_sword.current_state != evil_sword.State.DASH:
		return  # Only deal damage when in DASH state
	
	if animated_sprite.animation != "sword_attack":
		return  # Only deal damage during attack animation
	
	if animated_sprite.frame not in ATTACK_ACTIVE_FRAMES:
		return  # Only deal damage on active frames
	...
	if player and player.has_method("take_knockback"):
		# Calculate knockback direction: from evil sword to player
		var knockback_direction = (player.global_position - evil_sword.global_position).normalized()
